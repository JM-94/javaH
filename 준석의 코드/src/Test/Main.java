...
public class Main extends JFrame {
	...
	static String[] menu = { "NBB시그니처세트", "NBB오리지널세트", "NBB어메이징세트", "미트마니아세트", "메가바이트세트", "산체스세트", "스모키살사세트", "데리마요세트",
			...
			"상하이핑거포크S", "펩시콜라M", "사이다M", "미린다(오렌지)M", "오렌지주스M", "아메리카노Hot", "아메리카노Iced" };
	...
	public Main() {
		...
		order.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
//				table.getRow
				MyGetDate mgd = new MyGetDate();
				mgd.getTableData(table);
				JOptionPane.showMessageDialog(null, table, "정보", JOptionPane.INFORMATION_MESSAGE);
